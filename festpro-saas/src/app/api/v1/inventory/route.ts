...
export async function GET(request: NextRequest) {
  ...
  let q = admin.from("inventory_items").select("*").eq("organization_id", auth.organization_id)
  if (festivalId) q = q.eq("festival_id", festivalId)
  if (searchParams.get("category")) q = q.eq("category", searchParams.get("category"))
  if (searchParams.get("search")) q = q.or(`name.ilike.%${searchParams.get("search")}%,sku.ilike.%${searchParams.get("search")}%`)
